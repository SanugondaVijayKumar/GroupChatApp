...
async function creategroup(event){
   try{
    ...
    const response = await axios.post('http://16.171.165.252:3000/group/creategroup', groupDetails, {headers: {'Authorization': token}});
...
window.addEventListener('DOMContentLoaded', async () => {
    try{
        const response = await axios.get('http://16.171.165.252:3000/group/getallgroups', {headers: {'Authorization': token}});
