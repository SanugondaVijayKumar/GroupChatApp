...
addtogroup.addEventListener('click', async () => {
    try{
        ...
        console.log(obj);
        const response = await axios.post(`http://16.171.165.252:3000/content/addparticipant/${id}`, obj, {headers: {'Authorization': token}});
...
sendmsg.addEventListener('click', async () => {
    try{
        ...
        const response = await axios.post(`http://16.171.165.252:3000/content/sendmessage/${id}`, obj, {headers: {'Authorization': token}});
        if(response.status === 200)
        {
...
setInterval( async () => {
    try{
        ...
        const response = await axios.get(`http://16.171.165.252:3000/content/getgrpmessages/${id}`, {headers: {'Authorization': token}});
        if(response.status === 200){
            let len = '';
...
document.addEventListener('DOMContentLoaded',async () => {
    try{
        let id = window.location.href.split('=')[1];
        const response = await axios.get(`http://16.171.165.252:3000/content/grpparticipants/${id}`, {headers: {'Authorization': token}});
        if(response.status === 200){
            let lop = '';
            for(let i= 0;i<response.data.data.length;i++)
            {
...
grpparticipants.addEventListener('click', async (e) => {
    try{
        ...
        if(e.target.classList.contains('makeadmin'))
        {
            ...
            const response = await axios.post(`http://16.171.165.252:3000/content/makeuseradmin/${id}`, obj,{headers: {"Authorization": token}});
            alert(response.data.message);
            location.reload();
        }
        ...
        if(e.target.classList.contains('removeuser'))
        {
            ...
            const response = await axios.post(`http://16.171.165.252:3000/content/removeuser/${id}`, obj, {headers: {'Authorization': token}});
            alert(response.data.message);
            location.reload();
        }
    }
